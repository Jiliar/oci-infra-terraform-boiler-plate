...
data "oci_containerengine_cluster_kube_config" "cluster" {
  cluster_id = data.terraform_remote_state.prod.outputs.cluster_id
}
...
provider "kubernetes" {
  ...
  exec {
    ...
    command     = "oci"
    args        = ["ce", "cluster", "generate-token", "--cluster-id", data.terraform_remote_state.prod.outputs.cluster_id]
  }
}
