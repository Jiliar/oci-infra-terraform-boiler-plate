# OCI Authentication
tenancy_ocid   = "ocid1.tenancy.oc1..aaaaaaaafwdjaijilkfvc37n27ki626n2v5tew5nfqsqmsbtcq7qnbwwrp5a"
user_ocid      = "ocid1.user.oc1..aaaaaaaayh4gi5uw62nwcemruqq7ejk3hf2me274ervncgkixi2nywl4jfja"
fingerprint    = "f7:86:c8:7c:f4:d8:06:98:b0:33:25:f3:ae:a2:1d:1c"
private_key    = <<EOF
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
EOF
region         = "sa-bogota-1"

# Istio Configuration
istio_version                    = "1.20.0"
enable_gateway                   = true
gateway_name                     = "addon-ai-prod-gateway"
gateway_namespace                = "default"
gateway_hosts                    = ["addon-ai.com"]
enable_virtualservice            = true
virtualservice_name              = "addon-ai-prod-vs"
virtualservice_namespace         = "default"
virtualservice_hosts             = ["addon-ai.com"]
virtualservice_destination_host  = "addon-ai-prod-service"
virtualservice_destination_port  = 80
enable_peer_authentication       = true
mtls_mode                        = "STRICT"

# TLS/SSL Configuration (Cert-Manager)
enable_tls                       = true
enable_https_redirect            = true
tls_secret_name                  = "prod-addon-ai-tls"  # Cert-Manager creará este secret